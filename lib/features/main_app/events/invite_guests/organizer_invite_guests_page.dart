...
class OrganizerInviteGuestsPage extends StatefulWidget {
  const OrganizerInviteGuestsPage({super.key});

  @override
  State<OrganizerInviteGuestsPage> createState() => _OrganizerInviteGuestsPageState();
}

class _OrganizerInviteGuestsPageState extends State<OrganizerInviteGuestsPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      print('Args: $args');
      // Fix: Always extract eventId from arguments if present, regardless of type
  ...
  Future<void> _inviteSelectedUsers() async {
    print('Inviting users: $_selectedUsers for event: $_eventId');
    if (_eventId == null || _selectedUsers.isEmpty) return;
    setState(() {
      _isInviting = true;
    });
    ...
    for (final user in _selectedUsers) {
      ...
      final success = await eventProvider.inviteToEvent(userId: userId, eventId: _eventId!);
    ...
    if (allSuccess) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Guests invited successfully!')),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Some invites failed. Please try again.')),
        );
      }
    }
  }
